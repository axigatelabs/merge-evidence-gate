...
| Reason | Fires on |
|--------|----------|
| `CI failure suppressed` | An **added** line matching `continue-on-error: true`, `\|\| true`, `--no-verify`, `set +e`, or `allow_failure` — anywhere in a CI-relevant file (workflows and CI configs, `Makefile`, `package.json`, `scripts/`, task runners), and in any other shell script, Dockerfile, or pipeline YAML when the same line runs a test command (`pytest \|\| true`). `docker volume rm x \|\| true` in a helper script is housekeeping and does not count; source code (`flag \|\| true` is an operator), test fixtures, bundles, and documentation are never scanned. |
